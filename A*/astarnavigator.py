	...
	while open:
		...
		for index, edge in enumerate(network):
			if current in network[index]:
				...
				rolling_g = g[current] + distance(current, adjacent)

				if adjacent in closed and rolling_g >= g[adjacent]:
					continue

				if adjacent not in closed or rolling_g < g[adjacent]:
					origin[adjacent] = current
					g[adjacent] = rolling_g
					f[adjacent] = g[adjacent] + distance(adjacent, goal)
...
def myUpdate(nav, delta):
	...
	if gates is not None: 
		if (rayTraceWorld(nav.agent.moveOrigin, nav.agent.moveTarget, gates) is not None):
			print "strike"
...
def myCheckpoint(nav):
	...
	gates = nav.world.getGates()
	if gates is not None: 
		if (rayTraceWorldNoEndPoints(nav.agent.moveOrigin, nav.agent.moveTarget, gates) is not None):
			nav.agent.stopMoving()
