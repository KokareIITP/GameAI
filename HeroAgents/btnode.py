...
class Sequence(BTNode):
	...
	def execute(self, delta = 0):
		...
		temp = BTNode.getChildren(self)
		if temp == 0:
			return True

		if BTNode.getCurrentIndex(self) < len(temp):
			result = temp[BTNode.getCurrentIndex(self)].execute(delta)
			if result == False: 
				BTNode.setCurrentIndex(self, 0)
				return False
			elif result is None:
				return None
			elif result and BTNode.getCurrentIndex(self) < len(temp):
				BTNode.setCurrentIndex(self, BTNode.getCurrentIndex(self) + 1) 
				return None
			elif result:	
				return True	
